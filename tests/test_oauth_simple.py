...
class TestGoogleOAuth:
    ...
    def test_google_login_route_redirects(self, client):
        ...
        if location.startswith('/'):
            ...
            if response.status_code == 302:
                ...
                assert 'google.com' in oauth_location or 'accounts.google.com' in oauth_location, f"Expected Google OAuth redirect, got: {oauth_location}"
    ...
    def test_no_infinite_redirect_loop(self, client):
        ...
        for i in range(3):
            ...
            location = response.headers.get('Location', '')
            assert '/login/google' not in location, f"Request {i+1} redirects to itself - infinite loop!"
            ...
            if location.startswith('http') and 'google.com' in location:
                ...
                if response.status_code == 302:
                    ...
                    assert '/login/google' not in new_location, f"Internal redirect {i+1} goes back to Google login - infinite loop!"
