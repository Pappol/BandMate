import pytest
from app import db
from app.models import User, Band, Song, SongProgress, SongStatus, ProgressStatus, SetlistConfig
from datetime import datetime, date

class TestAdvancedSetlistFeatures:
    """Test advanced setlist features including buffer percentages and time clustering."""
    ...
    def test_time_clustering(self, app, test_band):
        ...
        with app.app_context():
            ...
            # Test clustering
            assert config.get_clustered_duration(45) == 60  # Rounds to nearest 30-min interval
    ...
    def test_buffer_calculation(self, app, test_band):
        ...
        with app.app_context():
            ...
            new_song_duration = config.calculate_song_duration_with_buffer(180, is_learned=False)  # 3 minutes
            assert abs(new_song_duration - 3.6) < 0.001  # 3 * 1.2 = 3.6
            
            # Test learned songs buffer (10%)
            learned_song_duration = config.calculate_song_duration_with_buffer(180, is_learned=True)  # 3 minutes
    ...
    def test_break_configuration(self, app, test_band):
        ...
        with app.app_context():
            ...
            # Test break needed
            assert config.is_break_needed(90) == True
            assert config.is_break_needed(120) == True
            
            # Test break not needed
            assert config.is_break_needed(60) == False
            assert config.is_break_needed(45) == False
    ...
    def test_setlist_config_update(self, app, test_band):
        ...
        with app.app_context():
            ...
            # Verify changes
            updated_config = SetlistConfig.query.filter_by(band_id=test_band.id).first()
    ...
    def test_buffer_in_setlist_generation(self, app, test_band, test_user, test_song):
        ...
        with app.app_context():
            ...
            assert buffered_duration == original_duration * (1 + config.new_songs_buffer_percent / 100)
